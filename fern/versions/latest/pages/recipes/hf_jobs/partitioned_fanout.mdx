---
title: "Partitioned Multi-Job Generation"
description: ""
position: 2
---
Scale a seed-driven workload horizontally by splitting the seed dataset into N partitions and running one job per partition. Partitioning uses Data Designer's native [seed selection strategies](/concepts/seed-datasets) — every job runs the same script with a different `PartitionBlock` index:

```python
config_builder.with_seed_dataset(
    seed_source,
    selection_strategy=dd.PartitionBlock(index=k, num_partitions=N),
)
```

The jobs are fully independent: each reads only its own partition and writes its own output shard, so there is no cross-job coordination, and a failed partition is retried by resubmitting that single index.

This recipe submits the partitions to [Hugging Face Jobs](https://huggingface.co/docs/huggingface_hub/guides/jobs), but any scheduler that can run a Python script works the same way.

## Run the Recipe

Run locally over the bundled demo seed (72 topic/audience rows), selecting one partition:

```bash
uv run partitioned_fanout.py --partition-index 0 --num-partitions 4 --num-records 18
```

Fan out across 4 cloud jobs, collecting the shards in a single Hub dataset:

```bash
NUM_PARTITIONS=4
RECORDS_PER_PARTITION=18   # partition size = seed rows / NUM_PARTITIONS
REPO_ID=<your-username>/partitioned-fanout-demo

for k in $(seq 0 $((NUM_PARTITIONS - 1))); do
    hf jobs uv run --detach --flavor cpu-basic --timeout 1h --secrets HF_TOKEN \
        partitioned_fanout.py \
        --partition-index "$k" --num-partitions "$NUM_PARTITIONS" \
        --num-records "$RECORDS_PER_PARTITION" \
        --endpoint https://router.huggingface.co/v1 --model openai/gpt-oss-20b --api-key-env HF_TOKEN \
        --push-to-hub "$REPO_ID"
done
```

Each job uploads its output as `data/partition-<k>-of-<N>.parquet` in the same dataset repo, so the finished dataset is directly loadable — the `data/*.parquet` shards form one split. Monitor progress with `hf jobs ps` and `hf jobs logs <job-id>`.

To run against your own seed data instead of the demo seed, pass `--seed-path` (local CSV/Parquet/JSON) or `--hf-seed-path` (a dataset on the Hub, e.g. `datasets/<your-username>/<dataset>/data/*.parquet`).

<Note>
Sizing `--num-records`
With ordered sampling, generation cycles through the selected partition if `--num-records` exceeds the partition size. To make exactly one pass over the seed data, set `--num-records` to the partition size (seed rows divided by `--num-partitions`).
</Note>

<Tip>
Choosing a partition count
Each partition job is limited by the `max_parallel_requests` it can push to your endpoint, so N jobs multiply your effective request concurrency by N. Start small (2-4 partitions), verify one partition's output end to end, then scale N up. See [Architecture & Performance](/concepts/architecture-performance) for per-job concurrency tuning.
</Tip>

## What works today, and current limits

Everything in this recipe is demonstrated end to end:

- **Independent partitions** — each job reads only its `PartitionBlock` slice; no cross-job coordination, no shared state.
- **Either inference mode** — a remote OpenAI-compatible endpoint (the default here), or an inference server running inside each job on a GPU flavor. The second mode is what makes fan-out scale: each job carries its own engine, so generation throughput grows linearly with the number of partitions instead of queueing behind one shared endpoint's rate limits. When running a dedicated in-job server, raise `max_parallel_requests` (default 4) and set an explicit request `timeout` — the defaults are tuned for hosted endpoints.
- **Per-partition retry** — partition slices are deterministic, so resubmitting a failed index regenerates exactly the missing shard.
- **Direct-to-Hub collection** — shards land as `data/partition-<k>-of-<N>.parquet` in one loadable dataset.

Current limits, so you can plan around them:

- **The partition is the retry unit.** If a job dies mid-partition, that partition reruns from the start — Data Designer's row-group resume state lives on the job's local disk and does not survive the container.
- **No launcher/coordinator.** The fan-out loop above is plain shell; retries are resubmitted by hand (or by your own wrapper). Nothing manages the fleet for you yet.
- **Concurrent uploads can conflict at large N.** Jobs finishing simultaneously can collide committing to one dataset repo; at N ≤ 4 with staggered finishes this is rarely an issue — beyond that, write per-partition repos or add a merge step.
- **One request per cell.** Generation is online, request-per-cell inference; offline batched-inference engines aren't a supported backend.
- Multi-GPU serving inside a single job (tensor parallel) is untested with these recipes; single-GPU is verified.

<Info title="Download Recipe">
  [Download the complete recipe script](https://github.com/NVIDIA-NeMo/DataDesigner/blob/main/docs/assets/recipes/hf_jobs/partitioned_fanout.py)
</Info>
